...
from model import Kriterie, Vandstand, Dkhype, Upload_get
...
def all_uploads(conn) -> list[Upload_get]:
    try:
        with conn.cursor() as cur:
...
def upload(conn) -> Upload_get:
    try:
        with conn.cursor() as cur:
            ...
            return Upload_get(id=row[0], Datetime=row[1], note=row[2], sommer=row[3])
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
